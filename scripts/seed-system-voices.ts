...
const CANONICAL_SYSTEM_VOICE_NAMES = Object.keys(systemVoiceMetadata);
...
async function ensureSystemVoicesAssets() {
  let files: string[];
  try {
    files = await fs.readdir(SYSTEM_VOICES_DIR);
  } catch (error: any) {
    if (error?.code === "ENOENT") {
      throw new Error(
        `Missing system voice assets at ${SYSTEM_VOICES_DIR}. Create the scripts/system-voices directory and add .wav files before running seed.`
      );
    }
    throw error;
  }
  ...
  if (wavFiles.length === 0) {
    throw new Error(
      `No .wav files found in ${SYSTEM_VOICES_DIR}. Ensure scripts/system-voices contains the expected audio assets before seeding.`
    );
  }
}
...
async function deleteSystemVoiceAudio(key: string) {
  await r2.send(
...
async function seedSystemVoice(name: string) {
  ...
  try {
    ...
    await deleteSystemVoiceAudio(r2ObjectKey).catch(() => {});
...
async function main() {
  await ensureSystemVoicesAssets();
