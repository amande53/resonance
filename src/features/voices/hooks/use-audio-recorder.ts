...
export function useAudioRecorder() {
  ...
  useEffect(() => {
    ...
    // Run only on unmount to release recorder, timer, media stream, and waveform resources.
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      if (err instanceof DOMException && err.name === "NotAllowedError") {
        setError(
          "Microphone access denied. Please allow microphone access in your browser setting"
        );
      } else {
