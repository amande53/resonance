...
import { VOICE_CATEGORIES, VOICE_CATEGORY_LABELS } from "@/features/voices/data/voice-category";

const LANGUAGE_OPTIONS = locales.all
  .filter((l) => l.tag && l.name)
  .map((l) => ({
    value: l.tag,
  ...
  return (
    ...
    >
      <div
        className={cn(
          scrollable
            ? "no-scrollbar flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto px-4 pb-2"
            : "flex flex-col gap-6"
        )}
      >
